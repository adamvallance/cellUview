...
getPrerequisites(){
    ...
    sudo apt update
    sudo apt upgrade
    pkgs=(gcc cmake build-essential libtool autoconf unzip wget qtbase5-dev qtdeclarative5-dev  libgtest-dev qtbase5-dev qtchooser qt5-qmake qtbase5-dev-tools doxygen) #dont think these are needed anymore but coudl be: python3 python3-dev glibc-source)
